    ...
    <div className="card mt-4">
      ...
      <div className="card-body">
        ...
        {weather.map((weather, index) => (
          <div key={index}>
            <p className="card-text text-center display-3">
              {weather.description}
            </p>
          </div>
        ))}

        <h3 className="py-2 text-center text-secondary">
          Current temperatur {formatTemp(main.temp)}{" "}
          <i className="wi wi-celsius" />
        </h3>
        <p className="text-center">
          <span className="px-4">
            {" "}
            Min temperatur {formatTemp(main.temp_min)}{" "}
            <i className="wi wi-celsius" />{" "}
          </span>{" "}
          <span className="px-4">
            Max temperatur {formatTemp(main.temp_max)}{" "}
            <i className="wi wi-celsius" />
          </span>
        </p>
      </div>
